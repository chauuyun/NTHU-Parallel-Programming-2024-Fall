...
__global__ void fw_phase3(int *dist, int padded_n, int r, int numBlocks, int start);
...
int main(int argc, char *argv[])
{
    ...
    int numBlocks = (padded_n + BLOCK_SIZE -1) / BLOCK_SIZE; 
    ...
    #pragma omp parallel num_threads(2)
    {
        ...
        cudaMalloc((void **)&device_matrix[threadId], padded_n * padded_n * sizeof(int));
        cudaMalloc((void **)&deviceEdges[threadId], sizeEdges);
        cudaMemcpy(deviceEdges[threadId], hostEdges, sizeEdges, cudaMemcpyHostToDevice);

        initMatrix<<<dim3(padded_n / 64, padded_n / 64), dim3(32, 32)>>>(device_matrix[threadId], padded_n);
        {
            int blocks = (m + threads - 1) / threads;
            setEdges<<<blocks, threads>>>(device_matrix[threadId], deviceEdges[threadId], m, padded_n);
        }
        ...
        for (int blockId = 0; blockId < numBlocks; ++blockId)
        {
            ...
            #pragma omp barrier
            // Phase 1:
            fw_phase1<<<dim3(1, 1), dim3(32, 32)>>>(device_matrix[threadId], padded_n, blockId);
            // Phase 2: 
            fw_phase2<<<dim3(numBlocks, 2), dim3(32, 32)>>>(device_matrix[threadId], padded_n, blockId, numBlocks);
            // Phase 3: 
            fw_phase3<<<dim3(numBlocks, gridPhase3Size), dim3(32, 32)>>>(device_matrix[threadId], padded_n, blockId, numBlocks, start);
        }
...
__global__ void fw_phase2(int *dist, int padded_n, int r, int numBlocks)
{
    ...
    if (blockIdx.y == 0) {
        ...
        global_j = blockIdx.x * BLOCK_SIZE + j;
    } else {
        // pivot column
        global_i = blockIdx.x * BLOCK_SIZE + i;
        global_j = r * BLOCK_SIZE + j;
    }
...
__global__ void fw_phase3(int *dist, int padded_n, int r, int numBlocks, int start)
{
    if (blockIdx.x == r || blockIdx.y + start == r)
        return;
    ...
    const unsigned int &j = threadIdx.x;
    int B = r * 64;
    int global_i = i + (blockIdx.y + start) * 64;
    int global_j = j + B;
    ...
    global_i = i + (blockIdx.y + start) * 64;
    global_j = j + blockIdx.x * 64;
    I_32 = i + 32; J_32 = j + 32; GLOBAL_I_32 = global_i + 32; GLOBAL_J_32 = global_j + 32;
    GLOBAL_I_padded = global_i * padded_n;
